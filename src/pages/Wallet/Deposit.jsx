...
function Deposit() {
   ...
   const usdtQrPayment = async () => {
     try {
       ...
       console.log("qr for usdt payment:", res.data.data[0].orcode);
       setUsdtQr(res.data.data[0].orcode); // store QR image URL
     } catch (error) {
       console.log(error);
     }
   };
          ...
          <div className="mt-5 ">
            <div className="bg-white shadow-lg rounded-lg p-2">
              ...
              <div className="flex flex-col items-center gap-3 my-0">
                ...
                <label className="text-sm font-medium text-white bg-redLight px-4 py-2 rounded-md cursor-pointer hover:bg-red">
                  Upload Screenshot
                  {/* <input
                    type="file"
                    accept="image/*"
                    ...
                    onChange={(e) => {
                      const file = e.target.files[0];
                      if (file) {
                        setUsdtScreenshot(URL.createObjectURL(file));
                      }
                    }}
                    className="hidden"
                  /> */}
